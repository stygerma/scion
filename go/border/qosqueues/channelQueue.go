...
func (pq *ChannelPacketQueue) Police(qp *QPkt, shouldLog bool) PoliceAction {
	...
	if shouldLog {
		//log.infog("Overall available bandwidth per second", "MaxBandWidth", pq.tb.MaxBandWidth)
		//log.infog("Spent token in last period", "#tokens", pq.tb.tokenSpent)
		//log.infog("Available bandwidth before refill", "bandwidth", pq.tb.tokens)
	}
	...
	if shouldLog {
		...
		//log.infog("Tokens necessary for packet", "bytes", qp.Rp.Bytes().Len())
	}
