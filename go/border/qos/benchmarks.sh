...
for i in {1..10}; do
go test -run=xxx github.com/scionproto/scion/go/border/qos/queues -bench=BenchmarkPoliceQueue >> testdata/BenchmarkPoliceQueue.txt
echo "Run $i/10"
done

tput setaf 2; echo "Benchmarking qos CheckAction"; tput sgr0
for i in {1..10}; do
go test -run=xxx github.com/scionproto/scion/go/border/qos/queues -bench=BenchmarkCheckAction >> testdata/BenchmarkCheckAction.txt
echo "Run $i/10"
done

tput setaf 2; echo "Benchmarking qos Queue Single Packet Blocking"; tput sgr0
for i in {1..10}; do
go test -run=xxx github.com/scionproto/scion/go/border/qos/queues -bench=BenchmarkQueueSinglePacketBlocking >> testdata/BenchmarkQueueSinglePacketBlocking.txt
echo "Run $i/10"
done

tput setaf 2; echo "Benchmarking qos Queue Single Packet Non-Blocking"; tput sgr0
for i in {1..10}; do
go test -run=xxx github.com/scionproto/scion/go/border/qos/queues -bench=BenchmarkQueueSinglePacket >> testdata/BenchmarkQueueSinglePacket.txt
echo "Run $i/10"
done
