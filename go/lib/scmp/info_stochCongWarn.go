...
const (
	stochCWLen = 56 //everything expect for RAW of path TODO: increase back to 56 when spath can be included
)
...
func (i *InfoStochCW) Len() int {
	return stochCWLen + i.Path.Raw.Len() + util.CalcPadding(stochCWLen+i.Path.Raw.Len(), common.LineLen) //
}
...
func (i *InfoStochCW) Write(b common.RawBytes) (int, error) {
	...
	return util.FillPadding(b, bscCWLen, common.LineLen+i.Path.Raw.Len()), nil //
}
