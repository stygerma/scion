...
const (
	bscCWLen = 56 //Everything included except for Raw of Path TODO: increase back to 56 when spath can be included
)
...
func (i *InfoBscCW) Len() int {
	return bscCWLen + i.Path.Raw.Len() + util.CalcPadding(bscCWLen+i.Path.Raw.Len(), common.LineLen) //
}
...
func (i *InfoBscCW) Write(b common.RawBytes) (int, error) {
	...
	return util.FillPadding(b, bscCWLen+i.Path.Raw.Len(), common.LineLen), nil //
}
